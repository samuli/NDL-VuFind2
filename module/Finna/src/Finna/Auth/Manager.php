...
class Manager extends \VuFind\Auth\Manager
{
    ...
    public function getAuthClassForTemplateRendering()
    {
        ...
        $auth = $this->getAuth();
        if ($auth instanceof \Finna\Auth\Shibboleth
            && null !== ($idp = $auth->getIdp())
        ) {
            $tpl .= "_$idp";
        }
        return $tpl;
    ...
    protected function getAuth($name = null)
    {
        ...
        if (!isset($this->auth[$name])) {
            list($name, $auth) = $this->buildAuth($name);
            $this->auth[$name] = $auth;
        }
     ...
     * Returns an array with handler name and handler.
     *
    ...
    protected function buildAuth($method)
    {
        ...
        if ($isNonDefaultShibboleth) {
            ...
            $this->pluginManager->configure(['shared_by_default' => true]);

            $type = substr($method, strlen($prefix));
            $auth->setIdp($type);
        } else {
            $auth = parent::makeAuth($method);
        }
